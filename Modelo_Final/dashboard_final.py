...
st.write("Conectado en vivo a la Base de Datos del pozo.")

ARCHIVO_BD = "C:\\Users\\andre\\IA-PYTORCH-N8N-ETC\\base_de_datos_pozo.csv"
...
if not os.path.exists(ARCHIVO_BD):
    ...
    if len(df) > 0:
        ...
        col2.metric("Vibración", f"{ultimo_dato['Vibracion']} Hz")
        ...
        # 3. Dibujamos la gráfica con los últimos 30 datos
        st.subheader("📈 Monitoreo de Sensores")
        # Seleccionamos solo las últimas 30 filas para el gráfico
        df_reciente = df.tail(30)
        st.line_chart(df_reciente[["Temperatura", "Vibracion"]])

# 4. El truco mágico: Recargar toda la web automáticamente cada 3 segundos
time.sleep(3)
st.rerun()